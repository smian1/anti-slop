...
Sibling of prose `anti-slop` (repo root). Load `references/` only when needed. Load a `recipes/` file when the surface matches.
...
| **Detect** | Audit / "is this AI UI?" | Name pattern, cite file/selector/region, short fix. No rewrite unless asked. No fake scores. |
...
2. **No font monoculture** — Ban unearned Inter / Roboto / Open Sans / unstated system-sans as both display and body. Use DESIGN.md or project fonts. Do not crown a replacement default.
...
5. **Decoration earns meaning** — Side-tab stripes, eyebrows/kickers, 01/02/03 theater, emoji/sparkle icons: keep only when semantic; otherwise cut.
...
7. **Cream kit is also a tell** — Unearned cream + serif + terracotta / Claude-beige kit ranks with purple SaaS. Brand override only.
...
| Inter / Geist / system sans | Ban unearned default; project fonts win; no new monoculture |
...
| Detect output | Pattern + cite + fix — no authorship accusation, no numeric "AI score" |
